...
void handleRoot() {
  ...
  <head>\
    ...
    <style>\
      ...
      .off { background: #333; }\
      .on { background: red; }\
      button { font-size: 20px; padding: 10px; margin: 5px; background: darkred; color: white; border: none; cursor: pointer; }\
...
void handleSendCommand() {
  if (server.hasArg("code")) {
    ...
    if (code == CMD_STOP_M) {
      stopRequested = true;
      stopTime = millis();
      server.send(200, "text/plain", "ESP_M stopped");
    } else if (code == CMD_STOP_L) {
      Serial.write(CMD_STOP_L);
      server.send(200, "text/plain", "Sent stop command to ESP_L");
    }
  } else {
